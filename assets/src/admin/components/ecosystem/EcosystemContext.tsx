...
export const EcosystemProvider = ({ children }: { children: any }) => {
  ...
  const restWarning: string = React.useMemo(() => {
    ...
    return VARS.activePlugins.indexOf(
      ...
      ? __(
          'The Plugin "Password Protected" is active and enabled. Therefore the REST API Access is restricted and features like the "push to deploy URL" won\'t work. Please allow REST API Access under "Protected Permissions".',
          'shgi'
        ) +
      ...
      : __(
          'REST API Access seems to be restricted. Therefore features like the "push to deploy URL" won\'t work. Please make sure that the REST API can be accessed for visitors.',
          'shgi'
        );
  }, [showRestWarning]);
